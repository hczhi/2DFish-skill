...
四、下一步"
        ...
        <p class="dr-note">
          ...
          <b>已经生成的图不会跟着变色</b>（图是像素，不是变量）—— 要笔触和配色都统一，得在那几页的备图栏逐张点「AI 生成」，
          每张都是一次真实花费。往后新生成的图会用现在这套配色。
          <b>「视觉母题」是第五项，走的是另一条路</b>：它只写进<b>往后生成图</b>的提示词（整页背景图 / 单图那两路），
          <b>已经生成的图和页面上的 CSS 一点都不会变</b> —— 换了它要在那几页的备图栏逐张点「AI 生成」才看得见，每张都是一次真实花费。
        </p>
...
const styleMismatch = computed(() => {
  ...
  return `第 ${bad.join(' / ')} 页的图是用别的画风生成的（现在选的是 ${styleId.value}）。换画风不会自动重做已有的图 —— 要笔触统一就在那几页的备图栏逐张点「AI 生成」（每张都是一次真实花费）。`
})
...
function onFrameMsg(e: MessageEvent) {
  ...
  if (d.type === 'ppt-del-nope') {
    editNote.value = d.why === 'img'
      ? `这一块里有 ${d.n || 1} 张图，删不了 —— 图的序号是按这一页里的出现顺序算的，少一格之后下一次重新配图会把第 2 张贴进第 1 格（图文不符，而页面看起来完全正常）。要去掉这一块请走「重新生成这一页」。`
      : d.why === 'whole'
