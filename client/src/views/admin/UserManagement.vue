          ...
          <tr v-for="u in users" :key="u.id">
            ...
            <td>
              <div class="table-actions">
                ...
                <button class="hc-btn hc-btn-secondary" @click="openDedicatedPage(u)">专属 AI</button>
              </div>
            </td>
          </tr>
        </tbody>
...
/** 专属 AI 配置是整页表单（三档 + 按应用两层），塞在弹窗里操作列会被挤出可视区。 */
function openDedicatedPage(u: UserRow) {
  router.push({ name: 'admin-dedicated-ai', params: { id: u.id } })
}
...
.token-panel h3 { font-size: 16px; font-weight: 700; color: #111827; margin: 32px 0 16px; letter-spacing: -0.3px; }
.token-panel h3:first-child { margin-top: 0; }
.token-panel h3 small { font-weight: normal; color: var(--c-text-sub); }
.token-panel .hc-table-container { margin-top: 12px; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.03); }
.mono { font-family: monospace; font-size: 12px; }
.empty-hint { font-size: 13px; color: var(--c-text-sub); }

.create-token-section { margin-top: 32px; padding-top: 24px; border-top: 1px solid #e5e7eb; }
.create-row { display: flex; gap: 16px; align-items: center; }
.create-row select { 
  flex: 1; padding: 12px 16px; border: 1px solid #e5e7eb; border-radius: 10px; font-size: 14px; 
...
.logs-section { margin-top: 32px; padding-top: 24px; border-top: 1px solid #e5e7eb; }
