...
import { apiStream, streamSSEData } from '../../lib/api'
...
async function batchGenerate() {
  ...
  try {
    const res = await apiStream('/api/discover/admin/articles/batch-generate')

    for await (const data of streamSSEData(res)) {
      if (data.done) {
