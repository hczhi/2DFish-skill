      ...
      <tbody>
        <tr v-for="a in articles" :key="a.id">
          ...
          <td>
            ...
            <br><small>{{ getSummary(a) }}</small>
          </td>
          <td><code>{{ a.slug }}</code></td>
          <td>
            <span class="badge badge-blue" v-for="l in a.visible_locales" :key="l">{{ l }}</span>
            <span v-if="!a.visible_locales.length" class="badge badge-gray">未配置</span>
          </td>
          <td>
            <span class="badge" :class="a.status === 'published' ? 'badge-green' : 'badge-gray'">
              {{ a.status === 'published' ? '已发布' : '草稿' }}
            </span>
          </td>
          <td>
            <span v-if="a.static_generated_at" class="badge badge-green">已生成</span>
            <span v-else class="badge badge-gray">未生成</span>
          </td>
          <td class="actions-cell">
            <router-link :to="`/admin/discover/edit/${a.id}`" class="btn-sm" style="text-decoration: none; display: inline-block;">编辑</router-link>
            <button class="btn-sm btn-generate" @click="generateStatic(a)" :disabled="!a.visible_locales.length">生成</button>
            <button class="btn-sm btn-danger" @click="deleteArticle(a.id)">删除</button>
          </td>
        </tr>
      </tbody>
    </table>
