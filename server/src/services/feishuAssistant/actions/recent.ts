...
//
// ── 这一层为什么存在 ──
// 改飞书任务要 guid，而**模型不可能知道它**。这和 open_id 是同一类问题
// （见 people.ts），解法也一样：id 绝不经过 LLM，只从代码控制的来源里取 ——
// 这里的来源是我们自己的执行日志（建的时候存了 guid）。
//
...
//
// ── 歧义绝不自己挑一个 ──
// 和 resolvePerson 同一条原则。挑错的后果是改了另一件事，
// 而回帖说的是「已完成」—— 用户要过几天才发现。所以有多个候选就列出来让他重说。
//
// 曾经还有一个 `deletedBy` 参数，用来把已删除的日程从候选里剔掉（否则
// 「再取消一次」会拿死 id 去 patch，或者给全体参与人再发一遍取消通知）。
// 日程动作删掉之后它没有使用者了，所以一并去掉。**再加删除类动作时要把它加回来**：
// 少了墓碑，删过的东西仍然是候选，而这条路上的失败都长得像成功。
...
export interface RecentTarget {
  /** 建它的那条指令产生的结构化产物（guid、标题、链接都在里面） */
 ...
 * 匹配用的归一化：小写 + **去掉所有空白**。
 *
 * 去空白是有原因的：中文里空格是可有可无的，同一个标题两次说出来空格位置常常不一样。
 * 「xzy8 月飞书 skill 开发」（建的时候标题里带空格，多半是输入法自动加的）
 * 和「xzy8月飞书skill开发」（改的时候顺手打的）在用户眼里是**同一个东西**，
 * 而 `includes` 会判成不匹配 —— 于是错误话术变成一句自相矛盾的话：
 * 「没找到和「xzy8月飞书skill开发」对得上的任务。我最近帮你建过这些：
 *  · xzy8 月飞书 skill 开发」。用户看到自己要的东西就列在那儿却说找不到，
 * 只会认定助理坏了。
 *
...
function haystack(ref: RecentActionRef): string {
  return norm(`${titleOf(ref)}\n${pickStr(ref.data, 'summary') ?? ''}`);
}
 ...
 *   很早的日志行可能没存 `guid`，拿它去调接口只会撞一个莫名的参数错误。
 *
 * 没有「墓碑」参数：目前一个删除类动作都没有（见文件头）。加回来之前，
 * 删过的东西仍然会留在候选里。
 */
export function findRecentTarget(
  keyword: string | undefined,
  ctx: ActionContext,
  opts: { actions: string[]; label: string; requireKeys: string[] }
): RecentTarget {
  ...
  // 关键词同样归一化。空白差异不该决定成败，见 norm 的注释。
