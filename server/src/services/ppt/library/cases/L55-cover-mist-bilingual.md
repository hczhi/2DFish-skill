...
- **比例**：16:9，**一张全幅气氛照**（雾山、水面、园区远景那一路），**没有蒙版**（暗调靠照片自己），全部文字是白的（约 60–110 字）
...
- `<img>` 铺满 + `object-fit:cover`；**上面没有任何蒙版**（原来 `.l55-bg::after` 那层 `rgba(16,30,46,.62)` 已删 —— 它和「这一页的蒙版」滑块叠起来整本发灰）。
- **于是「照片自己够暗」是白字能不能读的唯一依赖**：暮色、雾、逆光、暗水面、夜景那一路；一张晴天亮图进来，标题压在亮处，出来是一页「标题若隐若现」的封面，一处都不报错。左半尤其要暗（文字全在左半）。不够暗时拉「这一页的蒙版」滑块到 20–35%。
...
- **图必须自己是暗的**（版式里一层蒙版都没有）：白字压在亮天空上只是「若隐若现」，不报错
...
- **与 L13 的区别**：L13 是居中堆叠（年份 + 巨字 + 双语居中），四周收暗；L55 是**左对齐**、不带蒙版、带落款
