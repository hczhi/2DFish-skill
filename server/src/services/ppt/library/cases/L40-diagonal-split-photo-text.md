...
- **核心手法**：整库唯一的**斜切分屏** —— 图不是矩形的半幅，而是 `clip-path:polygon(100% 0,52% 0,66% 100%,100% 100%)` 切出来的一块斜梯形；那条斜边上再压一条 3px 的品牌→accent 渐变高光（`.l40-edge`，`left:59%` + `rotate(-14deg)`）
...
- 底色写 `var(--c-ink-deep)`：**没有配图时这一块仍然是一块暗色斜梯形**（V4），页面完整；不写底色的话没图那一版是一块纯白斜块，看起来像渲染失败。
- 上面那层 `::after` 是 165° 的黑渐变（`.12` → `.56`）：右下角要压深，图注是白字。不压的话图注在亮天空上直接看不见，而它照样渲染。
...
- **只能贴右下角。** 挪到左下角会落到斜边的浅底那一侧 —— 白字在米底上直接看不见，而 HTML、CSS、`checkPage` 都不会说一个字。
...
.l40-img{position:absolute;inset:0;z-index:0;overflow:hidden;background:var(--c-ink-deep);clip-path:polygon(100% 0,52% 0,66% 100%,100% 100%)}
.l40-img img{width:100%;height:100%;object-fit:cover;display:block}
.l40-img::after{content:"";position:absolute;inset:0;background:linear-gradient(165deg,rgba(0,0,0,.12) 0%,rgba(0,0,0,.56) 100%)}
...
| 槽位 | 必填? | 模式 | 比例 | 构图要求 |
|------|-------|------|------|----------|
| `pXX_side` | 可选（没有就走 V4，暗色斜块同样完整） | case | 16:9 | 会被斜切成右侧一块斜梯形（顶边从 52% 起、底边从 66% 起）：**主体必须在画面右三分之二**，左侧和左下角会被切掉；右下角要能压深（图注是白字）；不要人脸特写贴左边、不要带文字的图 |
...
| V4 | 去掉图（`.l40-img` 里不放 `<img>`，留暗色斜块） | 找不到合适的实景图 |
| V5 | 高光线改纯 `var(--c-brand)`（去掉渐变） | 整份禁用双色渐变 |
| V6 | 图块底色换 `var(--c-accent-deep)`，图注改深墨字 | 冷色调整份 |
