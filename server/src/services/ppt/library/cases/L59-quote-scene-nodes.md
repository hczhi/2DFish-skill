...
- `<img>` 铺满 + `object-fit:cover`；`::after` 是一层 **180deg、上（.58）→ 中（.3，34% 处）→ 下（.72）** 的深色蒙版。
- **蒙版是上下两头浓、中间淡的**：金句在中间、节点在下三分之一。换成单向渐变（比如只从上往下压暗）之后，下面那排圆的白字压在亮处读不出来，而那一页照样渲染。
...
- **蒙版必须是上下浓中间淡**：换成单向渐变之后下面那排圆的白字读不出来，不报错
