...
- `.l66-lead` 一段引言 **3–4 行最稳**（数据行是 `margin-top:auto` 贴底的，引言只有 2 行时中间那段留白有 500px 左右，看着像「这一页没写完」；反过来 5 行起会顶到数据行），**`max-width:640px` 不许放宽**：21px 的字最多只能压在 11% 的照片上（右端 780px = 图的 12.7% 处，实测 alpha 29），再宽就糊，而它仍然是一段读得通的话，浏览器、`checkPage`、导出一处都不会说。
...
- **只能贴右下角**，那里是图的**浓部**（66% 之后是全不透明）。挪到左下角会落到羽化成浅底的那一侧 —— 白字在米底上直接看不见，而 HTML、CSS、`checkPage` 都不会说一个字。
