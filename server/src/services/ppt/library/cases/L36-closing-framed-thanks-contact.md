...
- **核心手法**：① 一张**压暗到 .32 的全幅气氛图**当底（`.l36-bg`，压在 `--c-ink-deep` 上，**没有另一层蒙版**）；② 中央一个**1px 细线框**（`.l36-frame`，左右各缩 16%，半透白底 `.04`）里三层居中：104px 衬线巨字（「谢谢」/「以上」/一句收尾）→ 加宽字距英文小字 → 一段 2–3 行收尾话；③ 底部一条细线上的**三栏联系信息**（`.l36-contact`，单位 / 联系方式 / 日期或场合）
...
- **没有蒙版**：原来 `.l36-bg::after` 那层 `rgba(0,0,0,.75)` 已删（和 `.32` 叠起来整页几乎全黑、发灰）。
  暗调全靠 `opacity:.32` + 深底，所以图**本身**就得是暗的远景/夜景；亮图进来白字会发糊，而页面一处不报错。
...
| 槽位 | 必填? | 模式 | 比例 | 构图要求 |
|------|-------|------|------|----------|
| `pXX_bg` | 可选（没有就走 V4，纯深底同样完整） | concept | 16:9 | 全幅气氛底（1920×1080，会被压到 `.32`，没有额外蒙版）：**远景 / 夜景 / 天际线 / 抽象肌理**，中段不要有主体（细框压在那儿）；不要人物特写、不要图表 |
