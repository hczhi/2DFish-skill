...
- **图槽位**：**1 个（可选）** —— 只能是深底说明块的**背景图**（`.l69-say-bg`，块内一层，不占格子）。
  四个数字格里**不放图**（两行是 `1fr 1fr`，塞进去会把大数字和说明挤出格子下沿）
...
### 1b. 说明块的背景图 · `.l69-say-bg`（可选）

- 结构：`.l69-say` 的**第一个**孩子，里面一张 `<img>`，带 `data-img-prompt` / `data-img-mode="case"`。
  不占网格格子，四个大数字一个像素都不动。**没有合适的实景图就整块删掉这一层**（深底纯色本来就成立），
  别为了填这一格随便写一句「相关配图」—— 那一格每生成一次都是一次真实花费。
- 「画什么」要点三样：**整体压暗、没有强主体、右上或上方留亮**。图在 `opacity:.34` 下垫在白字底下，
  高调发白的图会把标题吃掉（字还在，就是读不出来），有强主体的图被 40px 的 padding 和文字盖掉一半。
- 竖构图（`3:4`）：这一块是通高的窄块，给 16:9 的话左右被裁掉大半，回来那张图的主体基本不在画面里。
- 导出 pptx 时这张图是**直角矩形**压在圆角块上（四角会露出一点点深底）—— 网页上是圆角。
...
.l69-say{grid-row:span 2;background:var(--c-ink-deep);border-radius:14px;padding:44px 40px;display:flex;flex-direction:column;position:relative;z-index:0}
.l69-say-bg{position:absolute;inset:0;z-index:-1;border-radius:14px;overflow:hidden;pointer-events:none}
.l69-say-bg img{width:100%;height:100%;object-fit:cover;display:block;opacity:var(--say-a)}
...
<section class="slide" data-layout="L69">
  ...
  <div class="slide-inner">
    <div class="l69-grid">
      <div class="l69-say">
        <!-- 可选：说明块的背景图。没有合适的实景图就把这一整层删掉，别留空的 img -->
        <div class="l69-say-bg"><img src="/ppt-cases/ph-3x4.svg" alt="" data-img-prompt="{{画什么：整体压暗、没有强主体、上方留亮的实景}}" data-img-mode="case"></div>
...
**1 个（可选）**：`pXX_saybg` —— 深底说明块的背景图（`.l69-say-bg`，`case` 模式、`3:4` 竖构图）。
**只有这一处能放图**：四个数字格里塞图会把大数字和说明挤出格子下沿（压在下一格上，不是消失），
要「数字 + 配图」换 L5 的扁平信息图模式。

这一格是**可以不要的**：没有合适的实景图就把 `.l69-say-bg` 整层删掉（深底纯色本来就是这条版式的
默认样子）。留一个空的 `<img>` 在那儿的话，这一页在备图面板上多出一格永远是占位图的槽 ——
而「配全部图」会照它生一张，那是一次真实花费。
...
- **`--veil` 留 0**：那层黑在整页之上（`.slide::before`），说明块的背景图是**块内**那一层压不到它 ——
  拉高之后四块实色和大数字一起变灰，而图一点没变暗（要图更暗就往 `<section>` 上写 `--say-a`，缺省 `.34` 在 `:root`）
- **说明块的背景图**（可选那一格）：整体压暗 / 没有强主体 / 上方留亮的 `3:4` 实景；`.l69-say` 的
  `position:relative;z-index:0` 和那一层的 `z-index:-1` 不许动（详见「一、结构速览」下面那条和骨架注释）
