...
- **核心手法**：① 全幅中央透视构图（图本身即焦点）；② 双向收暗蒙版（左 0–35% + 右 65–100%）；③ 左 1/3 文字（英巨 + 中副 + 品牌色短线 + 双语正文）；④ 右 1/3 三栏数据（衬线大数字 + 中英小标）；⑤ 底部居中 4 点分页指示
- **是否全幅**：是（**不包 `.slide-inner`**，与 L2/L3/L11/L13 同级 fullbleed 处理）
- **原色（仅供还原参考）**：暗蒙版（黑半透）/ 白色文字 / 黄色品牌色短线 / 衬线大数字
...
- **构图铁律**：图**必须中央透视**——左右两侧自然偏暗（被蒙版强化），中央亮、有景深延伸感
- **蒙版铁律**：双向收暗（左 0–35% + 右 65–100%），中央透亮，让中央成为视觉焦点
