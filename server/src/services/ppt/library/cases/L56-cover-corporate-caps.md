...
- **比例**：16:9，**一张亮调实景照**（厂区、车间、办公楼、产线那一路），**没有幕帘**（原来那层浅色幕已删），文字全部是**深色**的（约 60–120 字）
...
- **是否全幅**：**是** —— 不包 `.slide-inner`（照片要铺满，文字用 `var(--pad-x)` 对齐）
- **只当整份第 1 页用**（`归属` 只写封面）
...
- `<img>` 铺满 + `object-fit:cover`；**上面没有幕帘**（原来 `.l56-bg::after` 那层浅色 `rgba(250,248,244,.80)` 已删 —— 它把照片洗成一片灰白）。
- **L55 是暗照片压白字，这一条是亮照片压深色字。** 这一条是十条去掉蒙版的版式里**唯一没有退路**的：
  「这一页的蒙版」那个滑块是**黑**的，拉了只会让深色文字更读不出来。
- 所以照片的要求是硬的：**亮调、高调、左半是天空或干净的浅色背景、主体（楼/设备/产线）在右半**。
  左半有结构线、暗部或深色主体的照片进来，那几行深灰字直接糊在上面，而封面每一层都渲染正常、一处不报错。
...
- **必须有一张亮调实景照、左半干净、主体在右半**：暗照片进来深色文字就读不出来（没有浅色幕帘垫着了，滑块又是黑的），而那一页照样渲染
...
- **与 L35 的区别**：L35 是左右分栏、右边一整张竖图；L56 是一张整幅亮调照片、深字直接压在它的左半上
