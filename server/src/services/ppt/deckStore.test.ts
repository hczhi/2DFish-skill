...
describe('ppt deckStore', () => {
  ...
  it('没带提纲原文的那次编辑不会把原文清掉，带了就换成新的那一段', () => {
    ...
    updatePlanPageOutline(deckId, 'u1', 1, { title: '我写的标题', points: ['旧一'] });
    expect(outlineOf()).toBe('1.1 地产板块\n营收 12.4 亿，同比 +8%');

    // 带了就换：不换的话下一次生成用的还是老原文，而界面上他改过的那一段已经在框里了。
    updatePlanPageOutline(deckId, 'u1', 1, { title: '我写的标题', points: ['旧一'], outlineText: '我粘进去的补充材料' });
    expect(outlineOf()).toBe('我粘进去的补充材料');
  });

  it('删中间一页之后，剩下每一页身上的画面/备图/蒙版/要求还跟着它自己', () => {
    savePlan(deckId, 'u1', { pages: [{ page: 1, title: '一' }, { page: 2, title: '二' }, { page: 3, title: '三' }] });
    for (const n of [1, 2, 3]) {
      savePageHtml(deckId, 'u1', { page: n, layoutId: `L${n}`, html: `<section class="slide">第${n}页</section>` });
      savePendingImages(deckId, 'u1', { page: n, images: [{ index: 1, url: `/uploads/p${n}.png` }] });
      savePageSetup(deckId, 'u1', { page: n, notes: `第${n}页的要求` });
      savePageVeil(deckId, 'u1', { page: n, opacity: n / 10 });
    }

    const r = deletePage(deckId, 'u1', 2);

    // 搬的是「行的页码」，所以一整页的状态整体跟着走。挨个搬内容的话漏掉哪一列都是
    // 「那一页的图/蒙版/要求留在了原地」，而每一页显示出来都是一页正常的幻灯片。
    const rows = listPages(deckId, 'u1');
  ...
  it('在中间插一页，后面每一页的画面/备图/要求跟着自己往后挪一位（新那一页是空的）', () => {
    ...
    const r = insertPage(deckId, 'u1', { after: 1, title: '插进来的', points: ['一条'] });
    ...
    const plan = JSON.parse(getDeck(deckId, 'u1')!.plan_json);
    expect(plan.pages.map((p: any) => [p.page, p.title])).toEqual([[1, '一'], [2, '插进来的'], [3, '二']]);
    expect(getDeck(deckId, 'u1')!.planned_total).toBe(3);
    // 版本号 +1（在路上那次生成必须被 409 拒掉），版式和模块名跟着前一页并如实报出来
    expect(getDeck(deckId, 'u1')!.plan_rev).toBe(r.planRev);
  ...
  it('删过页之后，规划里那几条跨页提示会被标成「可能已经不准」', () => {
    ...
    deletePage(deckId, 'u1', 1);

    // 不标的话这句话读起来仍然像是这份稿子现在的问题，而它说的那两页已经不在那儿了
    // （页码对不上，也没有一处会说）。
    const problems = JSON.parse(getDeck(deckId, 'u1')!.plan_json).problems;
    expect(problems[0]).toContain('改过页数/页序');
    expect(problems).toHaveLength(2);

    // 再删一次不会又攒一条一样的提示
    savePlan(deckId, 'u1', { pages: [{ page: 1 }, { page: 2 }, { page: 3 }], problems: ['连续同版式那句'] });
    deletePage(deckId, 'u1', 1);
    deletePage(deckId, 'u1', 1);
    expect(JSON.parse(getDeck(deckId, 'u1')!.plan_json).problems).toHaveLength(2);
  });
