...
export const PALETTES: DesignOption[] = [
  ...
  },
  {
    id: 'P-E',
    name: '克莱因蓝',
    hint: '克莱因蓝 + 珊瑚橘（页底纯白）。品牌发布、设计与文化、出海与海洋',
    // 四处取色的理由，改的时候别按「更蓝更亮更高级」来动：
    // ① 品牌色取 IKB 原色 `#002FA7`，**不往亮的方向调**。这一套里蓝是大面积实底（章节页整块
    //    色块、L60 那条 82px 通栏色带、H-C 那档页眉色块、圆按钮），上面压白字：IKB 是 10.7:1，
    //    提亮到 `#0A3FC0` 还有 8.4:1，再亮一档就开始逼近 4.5 那条线 —— 而浏览器、`checkPage`、
    //    导出全都不报错，只有肉眼觉得「这一页字有点发飘」。
    // ② 它和 P-B「沉稳蓝」是**两套不同用途**，不是深浅版。P-B 的 `#2A5DB0` 是政企汇报那种稳，
    //    这一套是品牌浓度。把它填成 P-B 的邻近色的话，下拉里两条 hint 读起来差不多、出来的
    //    稿子也分不出差别 —— 他挑哪条都行，等于白多了一档。
    // ③ 点缀色取珊瑚橘（IKB 的经典互补色，也是参考图里那艘货轮的那一点暖），但**橘只能当装饰
    //    和浅底**：`--c-accent` 在库里有 8 处当文字色用，要写字的一律走 `--c-accent-deep`
    //    （`#B83C22` 白底 5.7:1，页眉 kicker 和各版式的领句走的就是它）。反过来填（深橘当
    //    `--c-accent`、亮橘当 deep）的话，左上角那行模块名在白底上几乎看不见，而它确实在那儿。
    // ④ `--c-accent-on` 压**墨色**不压白：珊瑚橘对白字只有 3.2:1，对 `#131A2B` 是 5.4:1。
    //    照「反色就是白色」填的话，角标/圆按钮里的字读不出来，而每一页都是一页完整正常的幻灯片。
    vars: {
      '--c-brand': '#002FA7', '--c-brand-deep': '#001E6C',
      '--c-accent': '#F2603F', '--c-accent-deep': '#B83C22',
      '--c-brand-on': '#FFFFFF', '--c-accent-on': '#131A2B',
      '--c-bg': '#FFFFFF', '--c-bg-alt': '#EAEEFA',
      '--c-ink': '#484F62', '--c-ink-deep': '#131A2B', '--c-ink-soft': '#8A90A3',
      '--c-card': '#FFFFFF', '--c-hairline': 'rgba(10,20,50,.10)',
      '--bg-warm': '#FFFFFF', '--bg-cool': '#FFFFFF', '--bg-plain': '#FFFFFF',
      '--card-o': '#E7ECFB', '--card-b': '#FDEBE4',
      '--hl-o': 'rgba(0,47,167,.14)', '--hl-b': 'rgba(184,60,34,.14)',
      '--mask-rgb': '255,255,255', '--mask-rgb-alt': '255,255,255',
    },
  },
];
