...
- **`ai_logs.reasoning_tokens / finish_reason`（107）是「关思维链到底生效了没」在后台唯一看得见的
  证据，后台「AI 日志」列表上就要显示。** gateway 早就核出来了（`reasoning_tokens > 0` 时
  `console.warn`），但那句只在服务器 stdout 里 —— 线上那一屏里「关掉了」和「没关掉」长得一模一样：
  都是一行正常记录，只有耗时不同，而慢有十种解释（实测就靠这两列才定位到「后台勾了不深度思考、
  它照旧想满 6000 输出」）。两条边界：① **上游没报明细时存 NULL，不存 0** —— 显示成「思 0」读起来
  是「已经关掉了」，于是唯一有用的动作（换模型 / 点那条接入点的「测试」）被这个 0 排除掉；
  ② `finish_reason` 原样存，`length` 在列表上标「截断」（折成布尔的话「上游没给」和「没截断」
  合成一句，而正文断在半句上、接口 200）。流式入口不报这两个值（它拿不到 usage 明细）。
...
- **关思维链是唯一让一条路径快起来的开关（`GatewayOptions.noThinking`），实测同一条接入点
  ...
  网关回 400 就**只摘被点名的那一个键、留着其余的重发** —— 不重发的话换条接入点之后整个模块变成一句 400；发出去
...
- **「不认识这几个键」那种 400 一次只摘被点名的那一个键（`namedBadKey`），不是四个一起摘干净。**
  实测 newapi 那条网关拒的只有 `chat_template_kwargs` 一个，而一起摘掉的话另外三个本来管用的键
  也没发出去 —— 于是这条接入点上「不深度思考」**从没生效过**：后台勾着、连通 ✓、业务 200，
  只是每页要两分钟、输出顶满 `max_tokens` 偶发截断（`ai_logs.reasoning_tokens` 是唯一看得见的证据）。
  点名的键按**长度倒序**匹配（`thinking` 是 `enable_thinking` 的子串，正序会摘错键，然后下一轮
  还是同一个 400）；点不出名字、或者就剩最后一个也被拒，才摘干净重发并喊一句（那时候
  `outcome.strippedAll` 置上 = 这次跑的是思维链全开那一版）。**只摘一个的那一步不喊** ——
  它很可能就此关成功了，喊「跑的是慢那版」是句假话，管理员会去换一个本来不用换的模型。
- **上游 400 有两种，退法不同，别合成一支。** ①「不认识这几个键」（OpenAI 官方那句
  «Unrecognized request argument»）→ 摘掉被点名的键重发（见上一条）；②「这个模型**始终思考、
...
- **连通测试（`POST /providers/:id/test`）在主请求之后**照着 `NO_THINKING_FORMS` 逐种试**最小请求
  （`probeNoThinking`，一次最多 25 秒），把结论回在 `no_thinking.{verdict,note,form}` 里、
  并把试出来的发法落库（见下面 108 那条）。理由是上面那种失败
  **在配置阶段完全看不出来** —— glm-5.3-flash 连通 ✓、回复正常，而管理员唯一的线索是用户来说
  「提取出来的资料不全」，而每次失败都真扣一次额度。五个 verdict 各对应一句不同的话（能关=
  已记住哪种发法 / 发了但它照旧想了 `reasoning_tokens` 个=已换下一种、再点一次接着试 /
  这个模型关不掉=运行时退到 low、长文件有截断风险 / 这种发法的键全被拒=已换下一种 /
  上游压根不报 `reasoning_tokens`=测不出来，判据在「AI 日志」的思维链那一列）。
  三条边界：① 探测**直接跑业务那一段 `withNoThinking`**（键、400 识别、一个一个摘的退法都在里面），
  在 admin 里另写一遍的话后台会显示一个和业务实际发的对不上的假结论；**结论一律看
  `reasoning_tokens` 不看「键发出去了没」** —— 摘掉某个键之后剩下的很可能就关成功了，那时候
  还报「这条网关不认那几个键」的话管理员会去换一个本来不用换的模型（摘了哪个键写在 note 里，
  那是这条接入点的特性）；② 探测自己失败
...
- **勾了那个开关不等于关掉了，所以「用哪种发法关」是运行时自己试出来的，记在
  `ai_providers.no_thinking_form`（108）。** 候选在 `gateway.ts` 的 `NO_THINKING_FORMS`
  （四键齐发 → 逐个单发 → `reasoning_effort:'none'` …），每次调用后 `learnNoThinkingForm`
  回头核 `reasoning_tokens`：还在想（或这种发法的键被全拒）就把**下一种**记上，走到底记
  `'none-works'`。这是唯一的出路 —— 各家网关认的键不是同一个（实测同一家中转的不同模型都
  不一样），而管理员那边**没有任何线索**：勾着「已关闭」、连通 ✓、业务 200，只是慢十倍、
  顶满 `max_tokens` 偶发截断。五条边界：① 判据是 `reasoning_tokens`，不是「请求成功」
  （发出去不等于生效）；② **`reasoning_tokens` 是 null（上游不报这个明细）且没有键被拒时
  一步都不能动** —— 没判据还往下换的话每次调用换一种、永远停不下来，而每次都是一次真调用，
  更要紧的是实测那家中转恰好在**关掉时**才不报这个明细，动了就是把已经调好的换坏；
  ③ 成功也要写（后台那一列才看得见，也才不会因为以后调整候选顺序把调好的接入点换回去）；
  ④ 上游明说「这个模型始终思考」时直接记 `'none-works'` —— 再逐个试换来的是几次真慢的调用，
  而每次最后还是退到 `reasoning_effort: 'low'`；⑤ 换发法必须 `console.warn` 喊出成因
  （这一次是带思维链跑完的 = 慢/可能截断，只是下次换个键）。流式入口**学不了**（流式响应
  不带 usage 明细），它只跟着用已经学到的那种；`upsertProvider` 要按 `!== undefined` 保留
  这一列（它不在编辑表单里，清成 NULL 的话下次调用又从第一种开始试，现象只是「又慢了一次」）。
  连通测试（`probeNoThinking`）走的是**同一张表**，一次最多试 25 秒并把进度落库，所以
  「点一次测试」= 现在就把发法试出来，不用等业务调用慢几次才收敛。
