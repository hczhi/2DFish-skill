...
- **每行只写 `style="--bar-v:数值"`，就是右边那一列印出来的那个原始数字。不许写百分比、不许自己折算。**
  条长是浏览器用 `flex-grow:var(--bar-v)` / `flex-grow:calc(var(--bar-max) - var(--bar-v))` 分出来的。
  自己折算的那一条照旧是一根颜色正常、长度看起来很合理的条，而它旁边的数字仍然是对的 ——
  没有任何地方会报错，读的人只会照着条长得出反的结论。
  `--bar-v` 和右边印出来的数字对不上时，**代码按印出来的那个数改回去并出声**（`chartData.ts`）。
- **轨道上限 `--bar-max` 由代码算，不要写。** 写了也会被重算：**一组百分比一律钉 100**
  （满格才真的是满），其余按最大值往上取一档整齐刻度（19 → 20）。
  原来这里教的是「取所有数值里最大的那个」—— 那样最长的一条永远顶满轨道，而数据是百分比时
  这就是致命的：40% 那一行画出来是满格，看起来是 100%，右边却还印着「40%」，一处都不报错。
  百分比的单位写在 `.dt-unit`（「单位：%」）或每行的 `<i>%</i>` 里，两处都算 —— 一处都不写的话
  代码认不出这是百分比，会按整齐刻度排（画面不算错，只是满格不再是 100）。
...
<section class="slide" data-layout="L47">
  <div class="slide-inner">
    ...
    <div class="l47-rows"><!-- 这一层不写 style：轨道上限由代码算 -->
...
| V4 | ~~自己写 `--bar-max`~~ | 已废弃：上限一律由代码算（百分比 100 / 其余整齐刻度） |
...
- **条长不许自己算百分比**：每行 `--bar-v` 就写**右边印出来的那个原始数**，除法交给浏览器（`flex-grow`）—— 自己折算算错的那条是一根长度看起来很合理的条、旁边数字还是对的，一处都不报错，而结论是反的
- **`--bar-max` 不要写**（写了会被 `chartData.ts` 重算）：**百分比一律钉 100**，其余按最大值往上取一档整齐刻度。原来教的「取最大值」会让最长那条永远顶满轨道 —— 数据是百分比时，40% 画出来就是满格、看起来是 100%，而旁边印着的还是 40%
