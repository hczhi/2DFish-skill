...
- **`right:0` / `bottom:0` 两边都出血，不许给它留 `var(--pad-x)`。** 留了之后这一页是「右下角贴了一张小图」—— 这一条存在的理由就是那个出血的角，而画面上是一页完整正常的幻灯片，自检清单 和类名校验一处都不会说。
...
- 不包 `.slide-inner`，但**页眉照旧贴**（内容页，页眉由工作流统一贴 贴在左上角 44–100px 那条带上，落在浅底上）。结构模板里**不要写 `.slide-header`**。
