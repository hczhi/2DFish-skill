# L74 · `photo-overlay-number-metrics`（整页实景图压暗 + 左栏序号标头 + 巨标题 + 一段说明 + 底部一排指标，全是白字压在图上）
...
- **比例**：16:9，一张铺满整页的实景图（压暗 42%）+ 左栏一列白字，文字容量偏小（标头 + 标题 + 一段说明 + 3 个数字，约 140–200 字）
...
- `::after` 那层 `rgba(0,0,0,.42)` 是**烙在版式里的**，不是这一页的蒙版（`--veil`）。veil 缺省 0，
  靠它的话谁没给这一页写过 `veil`，那一页就是白字压在图的亮部上 —— 投影到会议室墙上
  才发现读不出来。要更暗再叠 veil（它在 `z-index:1`，压得到图、压不到 `z-index:2` 的 `.l74-txt`
  —— `.l74-wrap` 不写 z-index 就是为了让蒙版还能插在这两层中间）。
- **图必须是暗调的，主体和亮部都在右侧**：左 52% 是文字区。主体在左边的图被文字盖掉一半，
  高调发白的图（雪景、白墙、晴天大面积天空）压完 42% 还是浅的，白字照旧读不出来。
...
- `<em>` 英文标签一到三个词（`ABOUT US` / `OUR EDGE` / `WHAT WE DO`），`<small>` 中文 ≤6 字。
  两者是**同一件事的两种语言**，不是两条信息；也不许是下面大标题的复制（那是 自检清单 ⑧）。
...
.l74-bg::after{content:"";position:absolute;inset:0;background:rgba(0,0,0,.42)}
...
- **不许把这两层包进 `.slide-inner`**（哪怕包了照样好看）：`imageSpace` 估留白时只收「不是图的
  ...
  好好的、一处都不报错，一处都不会说，他只会一张张重生（每张真扣一次额度）。装进 `.l74-wrap`
...
- **`--veil` 只往上加，不许指望它**：版式自带 `.42` 的黑是地板；veil 缺省 0，拿它当唯一的压暗手段
  的话没写过 `veil` 的那几页就是白字压亮图
